...
export default function StatsScreen({ onSettings }) {
  ...
  async function loadStats() {
    ...
    try {
      ...
      const [totalXP, todayPts, streak, goals, habits] = await Promise.all([
        getTotalXP(),
        getTodayPoints(today),
        getStreak(today),
        getGoals(),
        getHabits(),
      ]);
      ...
      const [allActions, completionsIn30, logsIn30, weekPoints] = await Promise.all([
        getAllActionsForGoals(goalIds),
        getCompletionsInRange(thirtyDaysAgo, today),
        getLogsInRange(thirtyDaysAgo, today),
        getPointsByDate(weekDates.map(d => d.date)),
      ]);
      ...
      if (userId && displayName) {
        ...
        syncLeaderboard(userId, displayName, score);
      }
